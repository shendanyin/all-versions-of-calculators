...
class CalculatorApp:
    def __init__(self, root):
        ...
        # 创建输入表达式的标签和输入框
        self.input_label = tk.Label(root, text="输入:", width=4, height=1, bg="red")    # 标签
        self.input_label.grid(row=0, column=0, sticky=tk.W)

        self.input_entry = tk.Entry(root, width=50)     # 输入框
        self.input_entry.grid(row=0, column=1, columnspan=2)

        # 创建显示结果的标签和输出框
        self.result_label = tk.Label(root, text="结果:", width=4, height=1, bg="red")   # 标签
        self.result_label.grid(row=1, column=0, sticky=tk.W)

        self.result_var = tk.StringVar()    # 输出框
        ...
        # 创建计算按钮，并绑定计算方法
        self.calculate_button = tk.Button(root, text="计算", width=10, height=1, bg="red", command=self.calculate)
    ...
    def evaluate_syntax_tree(self, syntax_tree):
        if isinstance(syntax_tree, ast.Num):
            # 数字节点
            return syntax_tree.n

        elif isinstance(syntax_tree, ast.UnaryOp) and isinstance(syntax_tree.op, ast.USub):
            # 处理负数
            return -self.evaluate_syntax_tree(syntax_tree.operand)


        elif isinstance(syntax_tree, ast.BinOp):
            # 二元运算节点
            left_value = self.evaluate_syntax_tree(syntax_tree.left)
            right_value = self.evaluate_syntax_tree(syntax_tree.right)

            if isinstance(syntax_tree.op, ast.Add):
                # 加法
                return left_value + right_value
            elif isinstance(syntax_tree.op, ast.Sub):
                # 减法
                return left_value - right_value
            elif isinstance(syntax_tree.op, ast.Mult):
                # 乘法
                return left_value * right_value
            elif isinstance(syntax_tree.op, ast.Div):
                # 除法
                return left_value / right_value

            elif isinstance(syntax_tree.op, ast.FloorDiv):
                # 整除
                return left_value // right_value
            elif isinstance(syntax_tree.op, ast.Mod):
                # 求模
                return left_value % right_value
            elif isinstance(syntax_tree.op, ast.Pow):
                # 乘方
                return left_value ** right_value
